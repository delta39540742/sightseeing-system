...
| GET | `/api/monitor/check-incident` | Kiểm tra sự cố thực tế (Weather/Traffic) |
| POST | `/api/monitor/mock-incident` | **(Dev-only)** Tạo sự cố giả lập để test Replan |
...
## 8. Testing & Mocking (Chỉ dành cho Dev)

Hệ thống hỗ trợ các công cụ giả lập để test các kịch bản hiếm gặp (như thời tiết xấu) mà không cần đợi thực tế.

### 8.1 Giả lập sự cố ngoại cảnh (Test Replan)

Sử dụng endpoint sau để ép hệ thống tạo cảnh báo sự cố:
...
*Lưu ý: Endpoint này chỉ tồn tại khi `NODE_ENV` khác `production`.*

### 8.2 Giả lập Nhận diện địa danh (Landmark Recognition)
