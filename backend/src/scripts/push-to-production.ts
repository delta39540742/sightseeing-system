...
async function seedFile(
  ...
  console.log(`[PRODUCTION-PUSH]   ${label}: đọc ${places.length} địa điểm`);
...
async function main() {
  ...
  if (!fs.existsSync(dataDir)) {
    console.error(`[PRODUCTION-PUSH] Thư mục data không tồn tại: ${dataDir}`);
    process.exit(1);
  }

  const files = fs.readdirSync(dataDir).filter((f) => f.endsWith('.json'));
  if (files.length === 0) {
    console.error('[PRODUCTION-PUSH] Không tìm thấy file JSON nào trong thư mục data/');
    process.exit(1);
  }

  const done = loadProgress();
  const pending = files.filter((f) => !done.has(f));
  console.log(`[PRODUCTION-PUSH] Tìm thấy ${files.length} file, đã seed ${done.size}, còn lại cần push ${pending.length} file`);

  if (pending.length === 0) {
    console.log('[PRODUCTION-PUSH] Tất cả file đã được push lên Production thành công!');
    await pool.end();
    return;
  }

  // Ensure tags exist in production before seeding
  const tagClient = await pool.connect();
  try {
    console.log('[PRODUCTION-PUSH] Khởi tạo các nhãn (tags) mặc định...');
    ...
    console.error('[PRODUCTION-PUSH] Lỗi khi tạo tags:', err.message);
  } finally {
    tagClient.release();
  }
  ...
  for (const file of pending) {
    ...
    console.log(`[PRODUCTION-PUSH] Đang push file: ${file}...`);
    try {
      const { inserted, skipped } = await seedFile(pool, filePath, label);
      console.log(`[PRODUCTION-PUSH]   Hoàn tất ${label}: ${inserted} inserted, ${skipped} skipped`);
      markDone(file);
      totalInserted += inserted;
      totalSkipped += skipped;
      successCount++;
    } catch (err: any) {
      console.error(`[PRODUCTION-PUSH] [Lỗi] Gặp sự cố khi push file ${file}:`, err.message);
    }
  }

  await pool.end();
  console.log(`\n[PRODUCTION-PUSH] ==================================================`);
  console.log(`[PRODUCTION-PUSH] KẾT QUẢ ĐỒNG BỘ PRODUCTION:`);
  console.log(`[PRODUCTION-PUSH] Đã push thành công: ${successCount}/${pending.length} file`);
  console.log(`[PRODUCTION-PUSH] Tổng số bản ghi được thêm mới: ${totalInserted}`);
  console.log(`[PRODUCTION-PUSH] Tổng số bản ghi bị trùng (skipped): ${totalSkipped}`);
